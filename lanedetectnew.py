...
def fitCurve(img):
    ...
    if len(leftx) == 0 or len(lefty) == 0 or len(rightx) == 0 or len(righty) == 0:
        print("Warning: No lane line pixels found.")
        return None, None
...
# Function that gives pixel location of points through which the curves of detected lanes passes
...
def drawCurves(img, pts_left, pts_right):
    ...
    cv2.polylines(img, np.int32([pts_left]),
                  isClosed=False, color=(0, 0, 255), thickness=10)
    cv2.polylines(img, np.int32([pts_right]), isClosed=False, color=(
...
def radiusOfCurvature(img, left_fit, right_fit):
    ...
    left_radius = (
        (1 + (2 * left_fit[0] * y_eval + left_fit[1]) ** 2) ** 1.5) / (2 * left_fit[0])
    right_radius = (
        (1 + (2 * right_fit[0] * y_eval + right_fit[1]) ** 2) ** 1.5) / (2 * right_fit[0])
...
def informationWindow(left_radius, right_radius, radius):
    ...
    window = cv2.putText(window, text1, (50, 50),
                         cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2, cv2.LINE_AA)
    text2 = 'Left Curvature : ' + \
        str(left_radius) + ', Right Curvature : ' + str(right_radius)
    window = cv2.putText(window, text2, (50, 100),
                         cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2, cv2.LINE_AA)
    text3 = 'Average Curvature : ' + str(radius)
    window = cv2.putText(window, text3, (50, 150),
                         cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2, cv2.LINE_AA)
    return window
...
def addTurnInfo(img, radius):
    if radius >= 10000:
        img = cv2.putText(img, 'Go Straight', (50, 50),
                          cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2, cv2.LINE_AA)
    if radius >= 0 and radius < 10000:
        img = cv2.putText(img, 'Turn Right', (50, 50),
                          cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2, cv2.LINE_AA)
    if radius <= 0:
        img = cv2.putText(img, 'Turn Left', (50, 50),
                          cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2, cv2.LINE_AA)
    img = cv2.putText(img, '(1)', (1000, 50),
                      cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2, cv2.LINE_AA)
    img = cv2.putText(img, '(2)', (1000, 230),
                      cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2, cv2.LINE_AA)
    img = cv2.putText(img, '(3)', (1165, 230),
                      cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2, cv2.LINE_AA)
...
out = cv2.VideoWriter(r'C:\Users\aryan\Documents\DIP Proj\lane_detect.avi',
                      cv2.VideoWriter_fourcc(*'XVID'), 25, (1280, 720))
...
while True:
    ...
    polygon = [(int(width * 0.15), int(height * 0.94)), (int(width * 0.45), int(height * 0.62)),
               (int(width * 0.58), int(height * 0.62)), (int(0.95 * width), int(0.94 * height))]
    masked_img = regionOfInterest(processed_img, polygon)
    source_points = np.float32([[int(width * 0.49), int(height * 0.62)], [int(width * 0.58), int(height * 0.62)],
                                [int(width * 0.15), int(height * 0.94)], [int(0.95 * width), int(0.94 * height)]])
    destination_points = np.float32([[0, 0], [400, 0], [0, 960], [400, 960]])
    warped_img_size = (400, 960)
    warped_img_shape = (960, 400)
    warped_img = warp(masked_img, source_points,
                      destination_points, warped_img_size)
    kernel = np.ones((11, 11), np.uint8)
    opening = cv2.morphologyEx(warped_img, cv2.MORPH_CLOSE, kernel)

    left_fit, right_fit = fitCurve(opening)
    if left_fit is None or right_fit is None:
        print("Skipping frame due to no lane lines found.")
        continue
